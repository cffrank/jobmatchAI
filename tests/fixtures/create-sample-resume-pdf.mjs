/**
 * Script to generate sample resume PDF for testing
 * Run with: node tests/fixtures/create-sample-resume-pdf.mjs
 */

import fs from 'fs';
import path from 'path';
import { fileURLToPath } from 'url';

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

// Read the text content
const textContent = fs.readFileSync(
  path.join(__dirname, 'sample-resume.txt'),
  'utf-8'
);

// Create a minimal PDF with text content
// PDF structure is plain text with specific headers/formatting
const pdfContent = `%PDF-1.4
1 0 obj
<<
/Type /Catalog
/Pages 2 0 R
>>
endobj

2 0 obj
<<
/Type /Pages
/Kids [3 0 R]
/Count 1
>>
endobj

3 0 obj
<<
/Type /Page
/Parent 2 0 R
/Resources <<
  /Font <<
    /F1 4 0 R
  >>
>>
/MediaBox [0 0 612 792]
/Contents 5 0 R
>>
endobj

4 0 obj
<<
/Type /Font
/Subtype /Type1
/BaseFont /Helvetica
>>
endobj

5 0 obj
<<
/Length ${textContent.length + 100}
>>
stream
BT
/F1 10 Tf
50 750 Td
15 TL
${textContent
  .split('\n')
  .map(line => `(${line.replace(/[()\\]/g, '\\$&')}) Tj T*`)
  .join('\n')}
ET
endstream
endobj

xref
0 6
0000000000 65535 f
0000000009 00000 n
0000000058 00000 n
0000000115 00000 n
0000000261 00000 n
0000000342 00000 n
trailer
<<
/Size 6
/Root 1 0 R
>>
startxref
${500 + textContent.length}
%%EOF
`;

// Write PDF file
const pdfPath = path.join(__dirname, 'sample-resume.pdf');
fs.writeFileSync(pdfPath, pdfContent, 'binary');

console.log('✅ Sample resume PDF created at:', pdfPath);
console.log('📄 File size:', fs.statSync(pdfPath).size, 'bytes');
